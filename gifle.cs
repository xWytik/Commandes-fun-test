...
internal sealed class Gifle : IChatCommand
{
    public void Execute(GameClient session, Room room, RoomUser userRoom, string[] parameters)
    {
        ...
        userRoom.OnChat(string.Format(LanguageManager.TryGetValue("cmd.gifle.chat", session.Language), targetUser.Username), 16);
        targetUser.OnChat(string.Format(LanguageManager.TryGetValue("cmd.gifle.chat.target", session.Language), userRoom.Username), 16);

        userRoom.ApplyEffect(5, true); // Effet "gifle"
        userRoom.TimerResetEffect = 4;

        targetUser.ApplyEffect(5, true); // Effet "giflé"
        targetUser.TimerResetEffect = 4;
    }
}
